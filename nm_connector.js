...
var nm_connector = {
        ...
        afio.isInstalled(function(installed) {
            ...
            try {
                if (__is_full_path(args.path)) {
                    ...
                    // afio.append() rejects '..' traversal segments
                    f = afio.openNode(Storage.getChar("defdownpath"));
                    f.append(args.path);
                }
            } catch (e) {
                nm_connector.sendResponse(
                    clientId, "Invalid capture path: "+e.message, -1);
                return;
            }
    ...
    onPlay: function(clientId, args) {
        ...
        context.ensure(win_id);   // rebuild engine if the worker was restarted
        ...
        if (__is_full_path(name)) {
            ...
            file = afio.openNode(Storage.getChar("defsavepath"));
            var nodes = name.split(__psep()).reverse();
            try {
                while (nodes.length)
                    file.append(nodes.pop());   // rejects '..' traversal
            } catch (e) {
                nm_connector.sendResponse(
                    clientId, "Invalid macro path: "+e.message, -931);
                return;
            }
        }
    ...
    startServer: function(args) {
        ...
        this.port.onMessage.addListener(function(msg) {
            // lastError is not meaningful inside a port listener; use onDisconnect
            setTimeout(function() {
        ...
        this.port.onDisconnect.addListener(function() {
            if (chrome.runtime.lastError) {
                console.info("iMacros SI host disconnected: " +
                             chrome.runtime.lastError.message);
            }
            nm_connector.port = null;
        });
    ...
    sendResponse: function(clientId, message, errorCode, extra) {
        ...
        if (!this.port) {
            console.warn("iMacros: SI port closed, dropping response for "+clientId);
            return;
        }
