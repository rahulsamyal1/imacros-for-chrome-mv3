...
var Editor = {
    ...
    saveFile: function () {
        ...
        swSend("save", {save_data: save_data, overwrite: true});
...
window.addEventListener("load", function() {
    ...
    // MV3: the editor is opened by the service worker; read its macro from the
    // dialog handshake instead of a cross-window win.args reference.
    getDialogArgs(function(a, reqId) {
        if (!a || !a.overwrite)
            $("save-button").style.display = "none";
        Editor.init(a ? a.macro : null);
    });
});
