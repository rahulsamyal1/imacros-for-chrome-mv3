...
function MacroPlayer(win_id) {
    ...
    // MV3: sandbox EVAL results arrive from the offscreen document via
    // chrome.runtime.onMessage and are routed to onSandboxMessage() by bg.js.
}
...
MacroPlayer.prototype.onAuthRequired = function(details, callback) {
    // MV3: asyncBlocking -> deliver the result through the callback.
    if (!this.loginData) {
        // no ONLOGIN credentials: take no action, let the browser prompt
...
MacroPlayer.prototype.ActionTable["add"] = function (cmd) {
    ...
    } else if ( m = cmd[1].match(/^!extract$/i) ) {
...
MacroPlayer.prototype.ActionTable["imagesearch"] = function (cmd) {
    ...
    if (!__is_full_path(image)) {
        ...
        var default_dir = afio.openNode(Storage.getChar("defdatapath"));
...
MacroPlayer.prototype.ActionTable["onlogin"] = function (cmd) {
    ...
    var self = this;
    // decryptThen() handles every encryption type asynchronously (WebCrypto),
    // suspending for the master-password dialog when needed.
    this.decryptThen(password, function(plain) {
        self.loginData.password = plain;
        self.next("ONLOGIN");
    });
};


// MV3: unified async decryption for ONLOGIN / TAG password values.
// Resolves the master password (cached tmpkey, stored key, or via the
// master-password dialog), decrypts with WebCrypto, then calls proceed(plain).
// For "SET !ENCRYPTION NO" it passes the value through unchanged.
MacroPlayer.prototype.decryptThen = function(ciphertext, proceed) {
    var self = this;
    var type = this.encryptionType;
    ...
    var doDecrypt = function(masterPwd) {
        Rijndael.decryptString(ciphertext, masterPwd).then(
            function(plain) { proceed(plain); },
    ...
    if (type === "stored") {
        var pwd;
        try {
            pwd = decodeURIComponent(atob(Storage.getChar("stored-password")));
        } catch (e) {
            self.handleError(new RuntimeError(
                "No stored master password is set", 712));
            return;
        }
        doDecrypt(pwd);
    } else if (type === "tmpkey") {
        if (Rijndael.tempPassword) {
            doDecrypt(Rijndael.tempPassword);
        } else {
            // suspend and ask the user for the master password
            self.waitingForPassword = true;
            openMasterPasswordDialog(self.win_id, function(ok) {
                self.waitingForPassword = false;
                if (!ok) {
                    self.handleError(new RuntimeError(
                        "Password input has been canceled", 943));
                    return;
                }
                doDecrypt(Rijndael.tempPassword);
            });
        ...
        self.handleError(new RuntimeError(
            "Unsupported encryption type: " + type, 712));
    }
};
...
MacroPlayer.prototype.ActionTable["saveas"] = function (cmd) {
    ...
    var f = null;
    if (folder == "*") {
        f = afio.openNode(Storage.getChar("defdownpath"));
    } else {
    ...
    f.exists(function(exists, err) {
        ...
        if (type == "extract") {
            ...
            chrome.pageCapture.saveAsMHTML(
                ...
                function(data) {
                    // MV3: no FileReader in the service worker; Blob.text()
                    if (!data) {
                        mplayer.handleError(new RuntimeError(
                            "pageCapture.saveAsMHTML returned no data", 1));
                        return;
                    }
                    data.text().then(function(text) {
                        afio.writeTextFile(f, text, function(e) {
                            if (e) { mplayer.handleError(e); return; }
                            mplayer.next("SAVEAS");
                        });
                    }, function(err) {
                        mplayer.handleError(err);
...
MacroPlayer.prototype.ActionTable["set"] = function (cmd) {
    ...
    case "!filestopwatch":
        ...
        if (__is_full_path(filename) ) { // full path
            ...
            file = afio.openNode(Storage.getChar("deflogpath"));
...
MacroPlayer.prototype.ActionTable["tag"] = function (cmd) {
    ...
    var sendTag = function() {
        if (!self.playingAgain) {
            self.nattempts = self.tagTimeout || 1;
        }
        communicator.postMessage("tag-command", data, self.tab_id,
                                 self.onTagComplete.bind(self),
                                 self.currentFrame);
    };

    if (isPasswordElement && data.type == "content" && data.txt) {
        // decrypt the password value (async, WebCrypto) then send the command
        this.decryptThen(data.txt, function(plain) {
            data.txt = plain;
            sendTag();
        });
    } else {
...
MacroPlayer.prototype.profiler = {
    ...
    getResultingXMLFragment: function(mplayer) {
        if (!this.enabled)
            return "";
        // MV3: build the XML fragment as a string (no DOM in the service worker)
        var esc = function(s) {
        ...
        var out = "<Macro>";
        out += "<Name>" + esc(mplayer.currentMacro) + "</Name>";
        ...
        for (var i = 0; i < source.length; i++) {
            if (j < this.profiler_data.length &&
                ...
                out += "<Command type=\"" + esc(x.type) + "\"";
                if (x.timeout_threshold)
                    out += " timeout_threshold=\"" + esc(x.timeout_threshold) + "\"";
                out += ">";
                out += "<String>" + esc(imns.trim(source[i])) + "</String>";
                for (var y in x) {
        ...
        out += "<Start>" + esc(this.make_str(this.macroStartTime)) + "</Start>";
        out += "<End>" + esc(this.make_str(macroEndTime)) + "</End>";
        out += "<ElapsedSeconds>" + duration.toFixed(3) + "</ElapsedSeconds>";
        out += "<Status><Code>" + esc(mplayer.errorCode) + "</Code>" +
               "<Text>" + esc(mplayer.errorMessage) + "</Text></Status>";
        out += "</Macro>";
...
MacroPlayer.prototype.saveProfilerData = function() {
    ...
    if (this.profiler.file) { // file was set with !FILE_PROFILER
        if (__is_full_path(this.profiler.file)) {
            ...
            file = afio.openNode(Storage.getChar("defdownpath"));
...
MacroPlayer.prototype.showAndAddExtractData = function(str) {
    ...
    // MV3: open the extract popup via the service worker; it resumes the
    // player (waitingForExtract -> next) when the dialog closes.
    openExtractDialog(this.win_id, str);
};
...
MacroPlayer.prototype.loadDataSource = function(filename) {
    ...
    if (!__is_full_path(filename)) {
        if (this.dataSourceFolder)
            ...
            file = afio.openNode(Storage.getChar("defdatapath"));
        file.append(filename);
    } else {
...
MacroPlayer.prototype.do_eval = function (s, eval_id) {
    ...
    if (this.__eval_results[eval_id]) {
        ...
        // MV3: relay to the sandbox iframe hosted in the offscreen document
        evalInSandbox(this.win_id, eval_data);
...
// MV3: called by bg.js with the eval-result payload object (not a DOM event)
MacroPlayer.prototype.onSandboxMessage = function(x) {
    this.__eval_results[x.id] = {
        result: (typeof x.result === "undefined") ? "" : x.result
    };
