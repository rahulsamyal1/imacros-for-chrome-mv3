...
function Connector() {
    ...
    chrome.runtime.onMessage.addListener(
        function(msg, sender, callback) {
            ...
            // some handlers (e.g. tag-command) respond asynchronously
            return true;
        }
    );
}
...
Connector.prototype.postMessage = function(topic, data, callback) {
    ...
    if (callback)
        chrome.runtime.sendMessage({topic: topic, data: data}, callback);
    else
        chrome.runtime.sendMessage({topic: topic, data: data},
