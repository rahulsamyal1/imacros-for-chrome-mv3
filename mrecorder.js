...
Recorder.prototype.onRecordAction = function(data, tab_id, callback) {
    ...
    var self = this;
    var m, pwd_re = new RegExp("\\btype=input:password\\b.+content=(\\S+)\\s*$", "i");
    if (!(m = pwd_re.exec(cmd))) {
        this.recordAction(cmd);
        return;
    }

    // handle password recording (possibly encrypted, async via WebCrypto)
    var plaintext = m[1];
    var typ = Storage.getChar("encryption-type") || "no";

    var recordEncrypted = function(masterPwd) {
        Rijndael.encryptString(plaintext, masterPwd).then(function(cyphertext) {
            self.recordAction(
                cmd.replace(/(content)=(\S+)\s*$/i, "$1=" + cyphertext));
        }, function(e) {
            console.error(e);
            self.recordAction(cmd);   // fall back to recording as typed
        });
    };

    // Emit the "SET !ENCRYPTION X" line once, and ONLY once we are committed to
    // a mode, so the recorded label always matches the recorded content even
    // though encryption is now async (key decode / dialog cancel happen first).
    var emitLabel = function(label) {
        if (self.writeEncryptionType) {
            self.writeEncryptionType = false;
            self.recordAction(label);
        }
    };
    ...
    case "stored":
        ...
        try {
            ...
            // no usable stored key -> record unencrypted (label matches content)
            emitLabel("SET !ENCRYPTION NO");
            this.recordAction(cmd);
            return;
        }
        emitLabel("SET !ENCRYPTION STOREDKEY");
        recordEncrypted(pwd);
        break;
    case "tmpkey":
        var withTmp = function() {
            emitLabel("SET !ENCRYPTION TMPKEY");
            recordEncrypted(Rijndael.tempPassword);
        };
        if (Rijndael.tempPassword) {
            withTmp();
        } else {
            openMasterPasswordDialog(this.win_id, function(ok) {
                if (!ok) {          // user cancelled -> record unencrypted
                    emitLabel("SET !ENCRYPTION NO");
                    self.recordAction(cmd);
                    return;
                }
                withTmp();
            });
...
Recorder.prototype.onAuthRequired = function(details, callback) {
    // MV3: asyncBlocking. Resolve the encryption key, then open the login
    // dialog popup; the service worker supplies the auth credentials to the
    // callback and records the ONLOGIN command when the dialog returns.
    var self = this;
    var typ = Storage.getChar("encryption-type") || "no";
    var enc = { encrypt: false, key: null };
    var encLabel = null;

    if (typ === "stored") { enc.encrypt = true; encLabel = "SET !ENCRYPTION STOREDKEY"; }
    else if (typ === "tmpkey") { enc.encrypt = true; encLabel = "SET !ENCRYPTION TMPKEY"; }
    else { enc.encrypt = false; encLabel = "SET !ENCRYPTION NO"; }

    if (this.writeEncryptionType) {
        this.writeEncryptionType = false;
        this.recordAction(encLabel);
    }

    var withKey = function(key) {
        enc.key = key;
        // openLoginDialog (defined in bg.js) holds the auth callback in the
        // service worker keyed by request id and records ONLOGIN on result.
        openLoginDialog(self.win_id, details, enc, callback);
    };
    ...
    if (!enc.encrypt) {
        ...
        if (Rijndael.tempPassword) {
            withKey(Rijndael.tempPassword);
        } else {
            openMasterPasswordDialog(self.win_id, function(ok) {
                withKey(ok ? Rijndael.tempPassword : null);
            });
        }
    }
};
