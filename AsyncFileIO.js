...
var afio = (function () {
    ...
    // SECURITY: reject '..' path-traversal segments (replaces the broken
    // String.replace("..","_") sanitization that only stripped the first ".."),
    // and fix the original 'legnth' typo.
    NodeObject.prototype.append = function(bit) {
        bit = String(bit);
        while (bit.length && (bit[0] == "/" || bit[0] == "\\"))
            bit = bit.substring(1);
        var parts = bit.split(/[\\/]+/);
        for (var i = 0; i < parts.length; i++) {
            if (parts[i] === "..")
                throw new RuntimeError(
                    "Illegal '..' path component in \"" + bit + "\"", 1);
        }
        var last = this._path.charAt(this._path.length - 1);
        this._path += (last == "/" || last == "\\") ? bit : __psep() + bit;
    };
    ...
    */
    obj.getDefaultDir = function(name, callback) {
        ...
        if (Storage.isSet("def"+name)) {
            if (!callback || typeof callback != "function")
                return;
            callback(this.openNode(Storage.getChar("def"+name)));
            return;
        }
