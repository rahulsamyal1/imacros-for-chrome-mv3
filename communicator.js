...
Communicator.prototype.addListeners = function() {
    ...
    chrome.runtime.onMessage.addListener(
        function(msg, sender, callback) {
            ...
            // handleMessage defers through chrome.tabs.get before invoking
            // the callback, so keep the sendResponse channel open.
            return true;
        }
    );
    ...
    function(topic, data, tab_id, callback, frame)
{
    chrome.tabs.sendMessage(
        ...
        callback || function() { void chrome.runtime.lastError; }
    );
};
...
Communicator.prototype.broadcastMessage = function(topic, data, win_id) {
    if (win_id) {
        chrome.tabs.query({windowId: win_id}, function(tabs) {
            if (!tabs)
                return;
